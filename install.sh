...
if [ -t 0 ]; then
  ...
  # cwd is pre-trusted so no trust prompt.
  ilog info wizard_handoff "exec claude (interactive tty)"
  ...
  echo ""
  if [ -x "$LAUNCHER" ]; then
    c_green "✓ Setup ready. Start the wizard by running:"
    echo ""
    echo "    $LAUNCHER"
    echo ""
    c_dim "(Run it in your terminal — keyboard works there, and the folder is already trusted."
    c_dim " A piped 'curl | bash' install can't take over the keyboard for the interactive wizard.)"
  else
    c_blue "Setup ready. Start the wizard with:"
    echo ""
    echo "    cd $INSTALLER_DIR && claude \"$WIZARD_PROMPT\""
    echo ""
  fi
  echo ""
  exit 0
